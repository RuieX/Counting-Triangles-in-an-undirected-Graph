...
// g++-6 -std=c++11 -march=native -O3 matrix-mult-bw-true.cpp -o mm-bw-true
...
int main (int argc, char* argv[]) {
	...
	std::vector<double> a_blockwise(size); // row-major
	...
	std::vector<double> b_blockwise(size); // row-major
	// iterate over c_row blocks
	long b_i = 0;
	for (long c_row_block=0; c_row_block<n_blocks; c_row_block++) {
		// iterate over c_col blocks
	...
	for (long c_row_block=0; c_row_block<n_blocks; c_row_block++) {
		...
		for (long c_col_block=0; c_col_block<n_blocks; c_col_block++) {
			...
			for (long i_block=0; i_block<n_blocks; i_block++) {
				long c_start = (c_row_block*n_blocks + c_col_block) * block_size;

				for (long i=0; i<block; i++) { // for each "row" in a
					long b_start = (c_col_block*n_blocks + i_block)* block_size;
					for (long j=0; j<block; j++) { // for each "col" in a
						long a_start = (c_row_block*n_blocks + i_block)* block_size + i*block;
						for (long k=0; k<block; k++) { // for each item in the row/col
							c[c_start] += a_blockwise[a_start++] * b_blockwise[b_start++]; 
						}
						c_start++;
					}
				}
