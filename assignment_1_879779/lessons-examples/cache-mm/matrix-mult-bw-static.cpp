...
// g++-6 -std=c++11 -march=native -O3 matrix-mult-bw-static.cpp -o mm-bw-static
	...
	for (long c_row_block=0; c_row_block<STATIC_N_BLOCKS; c_row_block++) {
		...
		for (long c_col_block=0; c_col_block<STATIC_N_BLOCKS; c_col_block++) {
			...
			for (long i_block=0; i_block<STATIC_N_BLOCKS; i_block++) {
				...
				for (long i=0; i<STATIC_BLOCK; i++) { // for each "row" in a
					long b_start = (c_col_block*STATIC_N_BLOCKS + i_block)* STATIC_BLOCK_SIZE;
					
					for (long j=0; j<STATIC_BLOCK; j++) { // for each "col" in a
						long a_start = (c_row_block*STATIC_N_BLOCKS + i_block)* STATIC_BLOCK_SIZE + i*STATIC_BLOCK;

						// below should be properly unrolled
						for (long k=0; k<STATIC_BLOCK; k++) { // for each item in the row/col
